...
from lib.string_manipulation import trim_length_from_end
...
class SMExecutionManager:
    ...
    def run_execution_sequential_mode(self):
        ...
        for sm_input in self.sm_input_list:
            ...
            if template_matched and parameters_matched and self.stack_set_exist:
                    start_execution_flag = not(self.check_stack_instances_per_account(sm_input, stack_set_name))
            else:
    ...
    def setup_execution(self, sm_input, name):
        ...
        return self.state_machine.trigger_state_machine(os.environ.get('SM_ARN'),
                                                        sm_input,
                                                        exec_name)
