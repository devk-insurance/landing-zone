...
then
    ...
    then
        ...
        do
            ...
            if [ -d $y/template_constraints ];
            then
                echo "rsync  --ignore-existing --verbose --recursive $y/template_constraints/* template_constraints"
                rsync  --ignore-existing --verbose --recursive $y/template_constraints/* template_constraints
                echo "Template Constraints synced." >> $merge_script_report
            else
                echo "'template_constraints' directory not found in $y."
                echo "'template_constraints' directory not found in $y." >> $merge_script_report
            fi
            if [ -d $y/parameters ];
            then
...
# Build AVM template
log_level='info'
lambda_arn_ssm=$lambda_arn_ssm_key
master_avm_template='yes'
echo "Master AVM template generation started." >> $merge_script_report
echo "python3 avm_generator.py $log_level $lambda_arn_ssm manifest.yaml $master_avm_template"
python3 avm_generator.py $log_level $lambda_arn_ssm manifest.yaml $master_avm_template
# Check python script exit code
if [ $? -ne 0 ]
then
  echo "Master AVM template generation failed."
  echo "Master AVM template generation failed." >> $merge_script_report
  exit 1
else
  echo "Master AVM template generation completed." >> $merge_script_report
fi
echo "-----------------------------------------------------------------------------" >> $merge_script_report
...
then
    ...
    then
        ...
        do
            ...
            # Build Add-On AVM template if exists in the add_on manifest
            master_avm_template='no'
            echo "python3 avm_generator.py $log_level $lambda_arn_ssm $y/$add_on_manifest $master_avm_template"
            python3 avm_generator.py $log_level $lambda_arn_ssm $y/$add_on_manifest $master_avm_template
            # Check python script exit code
            if [ $? -ne 0 ]
            then
              echo "Add-On AVM template generation failed."
              exit 1
            fi
            echo "-----------------------------------------------------------------------------" >> $merge_script_report
        done
