...
from lib.string_manipulation import trim_string_from_front
...
class CFNParamsHandler(object):
    ...
    def _get_az(self, ec2, key_az, qty):
        ...
        description = "Contains random AZs selected by Landing Zone" \
                      "Solution"
        if key_az:
    ...
    def _create_key_pair(self, account, region, param_key_material=None,
        ...
        key_name = sanitize("%s_%s_%s_%s" % ('lz', account,
                                             region,
    ...
    def random_password(self, length, key_password=None, alphanum=True):
        ...
        if not param_exists:
            ...
            description = "Contains random password created by Landing Zone"\
                          " Solution"
