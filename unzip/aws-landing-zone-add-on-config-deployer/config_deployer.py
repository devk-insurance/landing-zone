...
def find_alias(alias_name):
    ...
    while alias_not_found:
        ...
        logger.info(response_list_alias)
...
def create_cmk_with_alias(alias_name, event_policy):
    ...
    logger.info('Response: Create Key')
    logger.info(response_create_key)
    key_id = response_create_key.get('KeyMetadata', {}).get('KeyId')
    response_create_alias = kms.create_alias(alias_name, key_id)
    logger.info('Response: Create Alias')
    logger.info(response_create_alias)
    return key_id
